...
def send_jobs_digest_email(force_db_only=False):
    ...
    if force_db_only:
        ...
        from actions_scraper.database import db
        classified_jobs = list(db.jobs.find().sort("posted_date", -1).limit(200))
    else:
        from actions_scraper.scraper import _execute_scraper_cycle
    ...
    from actions_scraper.database import get_safe_score
...
from actions_scraper.database import get_master_settings
