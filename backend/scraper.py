...
from backend.ai_engine import process_job_classification
from backend.ats_scraper import scrape_all_company_portals
...
# generate_job_id helper is now imported from backend.database
...
def _execute_scraper_cycle():
    ...
    try:
        ...
        if raw_jobs:
            try:
                from backend.database import save_raw_jobs
...
def run_scraper_cycle():
    ...
    from backend.database import get_all_users_profiles
...
def process_raw_jobs_batch(profile_settings=None):
    ...
    from backend.database import get_unprocessed_raw_jobs, save_cleaned_job, get_all_settings, get_target_companies
                ...
                if company_name:
                    ...
                    if cleaned_name.lower() not in target_names:
                        ...
                        if portal_url:
                            ...
                            from backend.database import add_target_company
