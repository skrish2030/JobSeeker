    ...
    <div className="min-h-screen flex items-center justify-center bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-indigo-900/20 via-[#0A0710] to-[#0A0710] p-4 font-sans">
      <div className="w-full max-w-[420px] bg-[#120F1A]/90 backdrop-blur-2xl border border-[#ffffff15] p-8 rounded-3xl shadow-[0_15px_50px_rgba(0,0,0,0.6)]">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-100 tracking-wide">JOBSEEKER</h1>
          <p className="text-gray-400 text-sm mt-1">Secure Job Portal</p>
        </div>

        {/* Tabs */}
        <div className="flex w-full mb-8 bg-[#1A1625] p-1 rounded-xl border border-[#ffffff10]">
          <button 
            type="button"
            onClick={() => setActiveTab('login')}
            className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all ${activeTab === 'login' ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-400 hover:text-gray-200'}`}
          >
            Sign In
          </button>
          <button 
            ...
            className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all ${activeTab === 'register' ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-400 hover:text-gray-200'}`}
          >
            Register
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          >
            ...
            ) : activeTab === 'login' ? 'Sign In' : 'Confirm Registration'}
          </button>
